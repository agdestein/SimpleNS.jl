...
"""
    Grid(x, y; boundary_conditions, T = eltype(x))

Create nonuniform Cartesian box mesh `x` × `y` with boundary conditions `boundary_conditions`.
"""
function Grid(x, y; boundary_conditions, T = eltype(x))
    α = 81
    ...
    Grid{T,2}(; α, Nx, Ny, xlims, ylims, x, y, xp, yp, hx, hy, gx, gy, params...)
end

"""
    Grid(x, y, z; boundary_conditions, T = eltype(x))

Create nonuniform Cartesian box mesh `x` × `y` × `z` with boundary conditions `boundary_conditions`.
"""
function Grid(x, y, z; boundary_conditions, T = eltype(x))
    α = 81
